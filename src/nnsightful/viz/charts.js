...
`?(n&&(e.push(Et(n)),n=""),e.push('<span style="color:#3b82f6">\\n</span>')):n+=l}return n&&e.push(Et(n)),e.join("")}function Et(t){return t.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function fn(t){if(t>=.95)return 1;let e=[.003,.005,.01,.02,.03,.05,.1,.2,.3,.5,1];for(let o of e)if(t<=o)return o;return 1}function St(t,e,o){let n=fe(t),l=n.length>0?n[0].values.length:o?.values.length??t.xLabels?.length??0;if(l===0)return null;let u=e.mode||"probability",a=e.minValue,h=e.maxValue;if(a===void 0||h===void 0||e.autoScale){let r=[];for(let g of n)if(!g.isOverlay)for(let L of g.values)L!==null&&r.push(L);if(o)for(let g of o.values)g!==null&&r.push(g);if(r.length===0)return{numLayers:l,minValue:e.logScale?1:0,maxValue:1,numLines:n.length};let f=1/0,s=-1/0;for(let g of r)g<f&&(f=g),g>s&&(s=g);if(e.centerYAxisAtZero){let L=Math.max(Math.abs(f),Math.abs(s))*1.1;a=-L,h=L}else if(e.logScale){if(a===void 0&&(a=Math.max(1,Math.floor(f))),h===void 0||e.autoScale){let g=Math.log(Math.max(1,f)),L=Math.log(Math.max(1,s)),p=L+.15*Math.max(1,L-g);h=Math.ceil(Math.exp(p))}}else a===void 0&&(a=u==="probability"?0:u==="rank"?Math.max(1,Math.floor(f)):Math.floor(f*.9)),(h===void 0||e.autoScale)&&(u==="rank"?h=Math.ceil(s*1.1):u==="probability"?h=e.autoScale?fn(Math.max(s,.001)):Math.min(s*1.1,1):h=s*1.1)}return{numLayers:l,minValue:a,maxValue:h,numLines:n.length}}function $t(t,e,o,n,l,u,a,h){let r=t.getContext("2d"),f=e.getBoundingClientRect(),s=window.devicePixelRatio||1;t.width=f.width*s,t.height=f.height*s,t.style.width=`${f.width}px`,t.style.height=`${f.height}px`,r.scale(s,s);let g=f.width,L=f.height,p=n.darkMode??!1,M=n.title,H=n.mode||"probability",F=n.invertYAxis??!1,z=n.centerYAxisAtZero??!1,U=n.logScale??!1,P=n.xAxisLabel||"Layer",N=n.yAxisLabel||"Probability",B=n.xRangeStart??0;B>=l.numLayers-1&&(console.warn(`xRangeStart (${B}) is >= numLayers-1 (${l.numLayers-1}), clamping to 0`),B=0);let v=n.showDataPoints??!0,y={top:M?48:24,right:24,bottom:56,left:72},I=g-y.left-y.right,O=L-y.top-y.bottom,D={margin:y,chartWidth:I,chartHeight:O,width:g,height:L},C={background:p?"#0a0a0a":"#fafafa",text:"#71717a",textMuted:p?"#52525b":"#a1a1aa",grid:p?"rgba(255,255,255,0.06)":"rgba(0,0,0,0.06)",axis:p?"rgba(255,255,255,0.12)":"rgba(0,0,0,0.12)",titleText:p?"#e4e4e7":"#27272a"};n.transparentBackground?r.clearRect(0,0,g,L):(r.fillStyle=C.background,r.fillRect(0,0,g,L)),M&&(r.fillStyle=C.titleText,r.font="500 14px 'Inter', -apple-system, BlinkMacSystemFont, sans-serif",r.textAlign="left",r.fillText(M,y.left,28));let te=l.numLayers-1-B,ce=m=>te<=0?y.left+I/2:y.left+(m-B)/te*I,ie=U?Math.log(Math.max(1,l.minValue)):0,be=U?Math.log(Math.max(1,l.maxValue)):0,pe=m=>{let S;if(U){let _=Math.log(Math.max(1,m));S=be-ie>0?(_-ie)/(be-ie):.5}else S=(m-l.minValue)/(l.maxValue-l.minValue);return F?y.top+S*O:y.top+O-S*O};r.setLineDash([4,4]),r.strokeStyle=C.grid,r.lineWidth=1;let ne=[],oe=l.maxValue-l.minValue,R=5;if(U){let m=new Set;for(let q=0;q<R;q++){let Z=ie+q/(R-1)*(be-ie),Y=Math.round(Math.exp(Z));m.has(Y)||(m.add(Y),ne.push(Y))}let S=Math.round(Math.exp(ie)),_=Math.round(Math.exp(be));m.has(S)||ne.unshift(S),m.has(_)||ne.push(_)}else if(H==="rank")for(let m=0;m<R;m++)ne.push(Math.round(l.minValue+m/(R-1)*oe));else for(let m=0;m<R;m++)ne.push(l.minValue+m/(R-1)*oe);if(ne.forEach(m=>{let S=pe(m);r.beginPath(),r.moveTo(y.left,S),r.lineTo(y.left+I,S),r.stroke()}),r.setLineDash([]),z){let m=pe(0);r.beginPath(),r.strokeStyle=p?"rgba(255,255,255,0.4)":"rgba(0,0,0,0.3)",r.lineWidth=1.5,r.moveTo(y.left,m),r.lineTo(y.left+I,m),r.stroke(),r.lineWidth=1}r.fillStyle=C.text,r.font="400 11px 'Inter', -apple-system, BlinkMacSystemFont, sans-serif",r.textAlign="right",r.textBaseline="middle",ne.forEach(m=>{let S=pe(m),_;H==="probability"?_=m.toFixed(2):H==="prob_diff"?_=m>=0?`+${m.toFixed(2)}`:m.toFixed(2):_=Math.round(m).toString(),r.fillText(_,y.left-16,S)}),r.textAlign="center",r.textBaseline="top";let Ee=o.xLabels&&o.xLabels.length>0,ye=Math.max(1,Math.ceil(l.numLayers/8));for(let m=0;m<l.numLayers;m+=ye){let S=ce(m);if(S<y.left-5||S>y.left+I+5)continue;let _=Ee?String(o.xLabels[m]??m):m.toString();r.fillText(_,S,y.top+O+12)}if((l.numLayers-1)%ye!==0){let m=Ee?String(o.xLabels[l.numLayers-1]??l.numLayers-1):(l.numLayers-1).toString();r.fillText(m,ce(l.numLayers-1),y.top+O+12)}r.strokeStyle=p?"rgba(255,255,255,0.35)":"rgba(0,0,0,0.25)",r.lineWidth=1.5,r.beginPath(),r.moveTo(y.left,y.top),r.lineTo(y.left,y.top+O),r.lineTo(y.left+I,y.top+O),r.stroke(),r.fillStyle=C.textMuted,r.font="500 10px 'Inter', -apple-system, BlinkMacSystemFont, sans-serif",r.textAlign="center",r.textBaseline="top",r.fillText(P.toUpperCase(),y.left+I/2,L-16),r.save(),r.translate(14,y.top+O/2),r.rotate(-Math.PI/2),r.textAlign="center",r.textBaseline="top",r.fillText(N.toUpperCase(),0,0),r.restore();let X=fe(o);function Q(m,S,_,q,Z){r.beginPath(),r.strokeStyle=S,r.lineWidth=_,Z!==void 0&&(r.globalAlpha=Z),q?r.setLineDash(q.split(",").map(Number)):r.setLineDash([]);let Y=!1;for(let le=0;le<m.length;le++){let A=m[le];if(A===null){Y=!1;continue}let J=ce(le),Se=pe(A);Y?r.lineTo(J,Se):(r.moveTo(J,Se),Y=!0)}r.stroke(),r.setLineDash([]),Z!==void 0&&(r.globalAlpha=1)}let he=p?"#3f3f46":"#d4d4d8";if(r.lineCap="round",r.lineJoin="round",X.forEach((m,S)=>{!u.has(S)||m.isOverlay||Q(m.values,he,2,void 0,.35)}),X.forEach((m,S)=>{if(u.has(S)||m.isOverlay)return;let _=m.color??V[S%V.length];Q(m.values,_,4,m.dashPattern,.15),Q(m.values,_,2,m.dashPattern),v&&m.values.forEach((q,Z)=>{if(q===null)return;let Y=ce(Z),le=pe(q),A=a?.lineIdx===S&&a?.layerIdx===Z;r.beginPath(),r.strokeStyle=_,r.lineWidth=A?2:1.5,r.arc(Y,le,A?5:3.5,0,Math.PI*2),r.stroke(),r.beginPath(),r.fillStyle=p?"#18181b":"#ffffff",r.arc(Y,le,A?3.5:2.5,0,Math.PI*2),r.fill()})}),X.forEach((m,S)=>{if(!m.isOverlay||u.has(S))return;let _=m.color??"#999";Q(m.values,_,1.5,m.dashPattern??"4,2",.7)}),h){let m=h.color??"#999";Q(h.values,m,1.5,h.dashPattern??"4,2",.7)}return D}function Ct(t){let e=document.createElement("div");return e.style.cssText="position:absolute;pointer-events:none;z-index:50;opacity:0;transition:opacity 0.1s;transform-origin:center center;",t.appendChild(e),e}function Ht(t,e,o,n,l,u,a,h,r){let{margin:f,chartWidth:s,chartHeight:g}=l,L=n.xRangeStart??0,p=a-1-L,M=n.logScale?Math.log(Math.max(1,h)):0,H=n.logScale?Math.log(Math.max(1,r)):0;if(t<f.left||t>f.left+s||e<f.top||e>f.top+g)return null;let F=fe(o),z=null,U=1/0,P=20;for(let N=0;N<F.length;N++){if(u.has(N))continue;let B=F[N];if(!B.isOverlay)for(let v=0;v<B.values.length;v++){let y=B.values[v];if(y===null)continue;let I=p<=0?f.left+s/2:f.left+(v-L)/p*s,O;if(n.logScale){let te=Math.log(Math.max(1,y));O=H-M>0?(te-M)/(H-M):.5}else O=(y-h)/(r-h);let D=n.invertYAxis?f.top+O*g:f.top+g-O*g,C=Math.sqrt((t-I)**2+(e-D)**2);C<U&&C<P&&(U=C,z={visible:!0,x:I,y:D,lineIdx:N,layerIdx:v,value:y,label:B.label,color:B.color??V[N%V.length]})}}return z}function Ke(t,e,o,n,l,u,a){if(!e){t.style.opacity="0";return}let h=n?"#27272a":"#fff",r=n?"#3f3f46":"#e4e4e7",f=n?"#fafafa":"#18181b",s=n?"#a1a1aa":"#71717a",L=e.x>o/2?"calc(-100% - 12px)":"12px";t.style.cssText=`position:absolute;pointer-events:none;z-index:50;opacity:1;left:${e.x}px;top:${e.y}px;transform:translate(${L},-50%);`;let p=String(l?l[e.layerIdx]??e.layerIdx:e.layerIdx);t.innerHTML=`
    ...
    `;let D=document.getElementById(n),C=document.getElementById(n+"_scroll"),te=document.getElementById(n+"_hdr"),ce=document.getElementById(n+"_nav"),ie=document.getElementById(n+"_lp_wrap"),be=document.getElementById(n+"_lp_token"),pe=document.getElementById(n+"_lp_box"),ne=document.getElementById(n+"_lp"),oe=document.getElementById(n+"_tt"),R=document.getElementById(n+"_popup"),Ee=document.getElementById(n+"_popup_hdr"),ye=document.getElementById(n+"_popup_body");document.body.appendChild(R);let X=null,Q=null,he=!0,m=!1,S=getComputedStyle(D).getPropertyValue("--ll-aspect-ratio").trim(),_=!S;_&&D.classList.add("ll-fill");let q=(()=>{if(!S||/^(unbounded|none|auto)$/i.test(S))return null;let i=S.split("/").map(d=>parseFloat(d.trim()));return i.length!==2||isNaN(i[0])||isNaN(i[1])||i[0]===0||i[1]===0?null:i[1]/i[0]})(),Z=De,Y=Pe;function le(){let i=y(),d=O().shownLayers.length;if(Z=d>0?Math.max(De,Math.floor((i-He)/d)):De,!_){Y=Pe;return}let c=C.clientHeight;Y=M>0&&M*Pe<c?Math.floor(c/M):Pe}function A(){let i=B(),d=v(),c=s.showGrid?d?"border-right:1px solid rgba(0,0,0,0.28);border-bottom:1px solid rgba(0,0,0,0.22);":"border-right:1px solid rgba(255,255,255,0.55);border-bottom:1px solid rgba(255,255,255,0.45);":"";if(_){let E=l.clientWidth;D.style.width=E>0?E+"px":"100%",D.style.maxWidth="100%",D.style.maxHeight="",C.style.maxHeight=""}else{D.style.width="",D.style.maxWidth="",C.style.maxHeight="";let E=l.clientWidth||D.clientWidth;if(q&&E>0){let G=D.offsetHeight-C.offsetHeight,ee=(G>0?G:140)+90;D.style.maxHeight=Math.max(ee,Math.round(E*q))+"px"}else D.style.maxHeight=""}le();let{shownLayers:x}=O(),b=x.length,w=Math.round(He+Z*b),k=`${He}px repeat(${b}, ${Z}px)`,$=`<div class="ll-hdr-row" style="display:grid;grid-template-columns:${k};height:${Ln+6}px;width:${w}px;min-width:${w}px;">`;$+='<div class="ll-corner">token</div>';for(let E of x)$+=`<div class="ll-hdr-cell">${a.layers[E]}</div>`;$+="</div>",te.innerHTML=$;let T=`<div class="ll-grid-inner" style="width:${w}px;min-width:${w}px;">`;for(let E=0;E<M;E++){let W=a.tokens[E],G=Ie(W),ee=E===s.selectedRow;T+=`<div class="ll-row" data-rowwrap="${E}">`,ee&&(T+='<div class="ll-row-rail"></div>'),T+=`<div class="ll-row-grid${ee?" ll-row-sel":""}" data-row="${E}" style="display:grid;grid-template-columns:${k};height:${Y}px;">`,T+='<div class="ll-row-label">',T+=G?'<span class="ll-bos-pill">bos</span>':`<span class="ll-cell-text" style="${ee?"font-weight:600;":""}">${ot(W)}</span>`,T+="</div>",x.forEach((Le,$e)=>{let j=a.cells[E][Le],ae=j.prob,we=F!==""&&j.token===F,qe=nt(ae,we?kn:i,d),xt=Tn(ae,d),ln=ae<.18,rn=s.dimLow&&ln?"opacity:0.55;":"",Ye=U(j.token),an=Ye?`box-shadow:inset 0 0 0 2px ${Ye};`:"",yt=E===0,Lt=E===M-1,wt=$e===0,kt=$e===b-1,ke="";yt&&wt?ke="border-top-left-radius:8px;":yt&&kt?ke="border-top-right-radius:8px;":Lt&&wt?ke="border-bottom-left-radius:8px;":Lt&&kt&&(ke="border-bottom-right-radius:8px;"),T+=`<div class="ll-cell${Ye?" ll-cell-pinned":""}" data-row="${E}" data-layer="${Le}" style="background:${qe};color:${xt};padding:0 6px;${rn}${c}${an}${ke}"><span class="ll-cell-text">${ot(j.token)}</span></div>`}),T+="</div></div>"}T+="</div>",C.innerHTML=T}function J(){let i=B(),d=v(),{stride:c,start:x}=O(),b=s.viewSize,w=b>=p,k=st(),$=b<=k[0],T='<span class="ll-nav-range-key">layers</span>';w?T+=`all ${p}`+(c>1?`<span class="ll-dim"> \xB7 every ${c}</span>`:""):T+=`${x}\u2013${x+b-1}<span class="ll-dim"> / ${p}</span>`+(c>1?`<span class="ll-dim"> \xB7 \u22481/${c}</span>`:"");let E="";for(let j=0;j<p;j++){let ae=H[j]||0,we=Math.max(8,Math.round(ae*92));E+=`<div class="ll-skyline-bar" style="height:${we}%;background:${nt(ae,i,d)}"></div>`}let W=x/p*100,G=b/p*100,ee=[];for(let j=0;j<p;j+=8)ee.push(j);ee[ee.length-1]!==p-1&&ee.push(p-1);let Le="",$e=Math.max(1,p-1);for(let j of ee){let ae=j===p-1,we=j===0,qe=j/$e*100;Le+=`<span class="ll-nav-tick" style="left:${qe}%;transform:${ae?"translateX(-100%)":we?"translateX(0)":"translateX(-50%)"}">${a.layers[j]}</span>`}ce.innerHTML=`
            ...
            <div class="ll-nav-controls">
                ...
                <button class="ll-nav-btn" data-nav="zoomOut" title="Zoom out" ${w?"disabled":""}>${Hn}</button>
                <button class="ll-nav-btn" data-nav="reset" title="Reset to overview">${En}</button>
            </div>
        `,Yt()}function Se(){let i=document.getElementById(n+"_win");if(!i)return;let d=Math.max(0,p-s.viewSize),c=Math.max(0,Math.min(d,s.viewStart));i.style.left=c/p*100+"%",i.style.width=s.viewSize/p*100+"%"}function st(){return Array.from(new Set([p,48,32,20,14,10,8])).filter(i=>i<=p&&i>=1).sort((i,d)=>i-d)}function Oe(i){return Math.max(0,Math.min(Math.max(0,p-s.viewSize),i))}let Ae=!1;function dt(){Ae||(Ae=!0,requestAnimationFrame(()=>{Ae=!1,!m&&(A(),Se())}))}function Re(i){let d=Oe(s.viewStart+i);d!==s.viewStart&&(s.viewStart=d,A(),J(),me())}function Gt(i){let d=Oe(i);d!==s.viewStart&&(s.viewStart=d,dt())}function ct(i,d){let c=st(),x=c.findIndex($=>$>=s.viewSize),b=x===-1?c.length-1:x,w=i<0?Math.max(0,b-1):Math.min(c.length-1,b+1),k=c[w];k!==s.viewSize&&(s.viewSize=k,s.viewStart=Math.max(0,Math.min(p-k,Math.round(d-k/2))),A(),J(),me())}function pt(i){ct(i,s.viewStart+s.viewSize/2)}function qt(){s.viewSize=p,s.viewStart=0,A(),J(),me()}let ue=null;function Yt(){let i=document.getElementById(n+"_sky");if(!i)return;i.addEventListener("pointerdown",c=>{let x=i.getBoundingClientRect(),b=c.clientX-x.left,w=x.width/p,k=Math.max(0,p-s.viewSize),$=Math.max(0,Math.min(k,s.viewStart)),T=$*w,E=($+s.viewSize)*w,W=$;(b<T||b>E)&&(W=Oe(Math.round(b/w)-Math.floor(s.viewSize/2)),s.viewStart=W,dt()),ue={startX:b,startStart:W,layerW:w},i.classList.add("ll-grabbing");try{i.setPointerCapture(c.pointerId)}catch{}}),i.addEventListener("pointermove",c=>{if(!ue)return;let x=i.getBoundingClientRect(),w=(c.clientX-x.left-ue.startX)/ue.layerW;Gt(Math.round(ue.startStart+w))});let d=c=>{if(ue){ue=null,i.classList.remove("ll-grabbing");try{i.releasePointerCapture(c.pointerId)}catch{}J(),me()}};i.addEventListener("pointerup",d),i.addEventListener("pointercancel",d),i.addEventListener("wheel",c=>{c.preventDefault();let x=i.getBoundingClientRect();if(Math.abs(c.deltaX)>Math.abs(c.deltaY)){Re(c.deltaX>0?1:-1);return}let b=Math.round((c.clientX-x.left)/x.width*p);ct(c.deltaY<0?-1:1,b)},{passive:!1})}function ht(i,d,c=!1){s.selectedRow=i,s.selectedLayerIdx=d??a.layers.length-1,A(),re(),c&&Xt(i),me()}function Xt(i){let d=C.querySelector(`[data-rowwrap="${i}"]`);if(!d)return;let c=d.offsetTop-C.clientHeight/2+d.offsetHeight/2;C.scrollTo({top:Math.max(0,c),behavior:"smooth"})}function We(){requestAnimationFrame(()=>requestAnimationFrame(()=>{m||(C.scrollTop=C.scrollHeight)}))}let Qt=.45,Zt=120,Jt=360;function Fe(){let i;if(q)i=(l.clientWidth||D.clientWidth)*q;else{let c=D.clientHeight;i=c>0?c:(D.clientWidth||900)*.6}let d=Math.round(i*Qt);pe.style.height=Math.max(Zt,Math.min(Jt,d))+"px"}function re(i){let d=s.selectedRow,c=[];if(d!==null)for(let b of s.pinned){let w=N(d,b.token);w&&c.push({values:w.map(k=>k??null),label:xe(b.token),color:b.color,removable:!1})}let x=c.length===0&&!i;if(x)ie.classList.add("ll-hidden");else{ie.classList.remove("ll-hidden"),Fe(),be.textContent=d===null?"":Ie(a.tokens[d])?"position "+d+" \xB7 bos":"position "+d+" \xB7 "+xe(a.tokens[d]);let b={lines:[],richLines:c,xLabels:a.layers},w={darkMode:v(),mode:"probability",autoScale:!0,legendPosition:c.length>1?"right":"none",showDataPoints:!0,xAxisLabel:"layer",yAxisLabel:"probability",transparentBackground:!0};X?(X.setData(b),X.setOptions(w)):(ne.style.minHeight="0",X=new se(ne,b,w)),i?X.setOverlay?.({values:i.values,label:i.label,color:i.color,dashPattern:"4,2",isOverlay:!0}):X.setOverlay?.(null)}_&&x!==he?(he=x,requestAnimationFrame(()=>{m||(A(),We())})):he=x}function Kt(i,d,c,x){let b=a.cells[i]?.[d];if(!b)return;let w=B(),k=Ie(a.tokens[i])?"bos":xe(a.tokens[i]);oe.innerHTML=`<div class="ll-tt-head"><span class="ll-tt-swatch" style="background:${nt(b.prob,w,v())}"></span><span class="ll-tt-token">${ve(xe(b.token))}</span></div><div class="ll-tt-grid"><span>probability</span><span class="ll-tt-val">${(b.prob*100).toFixed(1)}%</span><span>layer</span><span class="ll-tt-val">${a.layers[d]} / ${a.layers[p-1]}</span><span>position</span><span class="ll-tt-val">${i} \xB7 ${ve(k)}</span></div>`,oe.classList.add("ll-visible");let $=D.getBoundingClientRect(),T=oe.offsetWidth||220,E=oe.offsetHeight||90,W=c-$.left+16;c+T+24>window.innerWidth-8&&(W=c-$.left-T-12);let G=x-$.top-50;W=Math.max(6,Math.min(W,$.width-T-6)),G=Math.max(6,Math.min(G,$.height-E-6)),oe.style.left=W+"px",oe.style.top=G+"px"}function Ne(){oe.classList.remove("ll-visible")}function en(){je(),Q=document.createElement("div"),Q.style.cssText="position:fixed;inset:0;z-index:49;",Q.addEventListener("mousedown",i=>{i.preventDefault(),i.stopPropagation(),mt()}),document.body.appendChild(Q)}function je(){Q&&(Q.remove(),Q=null)}function tn(i,d,c){if(!a.cells[i]?.[d])return;let b=c.getBoundingClientRect();s.openPopup={row:i,layer:d},s.selectedRow=i,s.selectedLayerIdx=d;let w=Ie(a.tokens[i])?"bos":xe(a.tokens[i]);Ee.innerHTML=`Layer <b>${a.layers[d]}</b>, Position <b>${i}</b><div class="ll-popup-sub">input <code>${ve(w)}</code></div>`,ut(i,d),A(),re(),R.style.visibility="hidden",R.classList.add("ll-visible"),nn(b),R.style.visibility="",en(),me()}function ut(i,d){let c=a.cells[i][d],x="";c.topk.forEach((b,w)=>{let k=U(b.token),$=k?`background:${k}22;border-left-color:${k};`:"";x+=`<div class="ll-topk${k?" ll-topk-pinned":""}" data-ki="${w}" style="${$}" title="click to track trajectory"><span class="ll-topk-tok">${ot(b.token)}</span><span class="ll-topk-prob">${(b.prob*100).toFixed(1)}%</span></div>`}),ye.innerHTML=x,ye.querySelectorAll(".ll-topk").forEach(b=>{let w=parseInt(b.dataset.ki),k=c.topk[w];b.addEventListener("mouseenter",()=>{let $=N(i,k.token);$&&re({values:$.map(T=>T??null),label:xe(k.token),color:"#999"})}),b.addEventListener("mouseleave",()=>re()),b.addEventListener("click",$=>{$.stopPropagation(),P(k.token),A(),ut(i,d),re(),me()})})}function nn(i){let x=R.offsetWidth||220,b=R.offsetHeight||160,w=6,k=window.innerWidth-x-6,$=6,T=window.innerHeight-b-6,E=[{left:i.right+6,top:i.top},{left:i.left-6-x,top:i.top},{left:i.left,top:i.bottom+6},{left:i.left,top:i.top-6-b}],W=E[0];for(let G of E)if(G.left>=w&&G.left<=k&&G.top>=$&&G.top<=T){W=G;break}R.style.left=Math.max(w,Math.min(W.left,k))+"px",R.style.top=Math.max($,Math.min(W.top,T))+"px"}function mt(){s.openPopup=null,R.classList.remove("ll-visible"),je(),A(),re()}document.getElementById(n+"_popup_close").addEventListener("click",i=>{i.stopPropagation(),mt()});let K=null;C.addEventListener("mousemove",i=>{let d=i.target.closest(".ll-cell");if(!d){K&&(K.classList.remove("ll-cell-hover"),K=null),Ne();return}K!==d&&(K&&K.classList.remove("ll-cell-hover"),K=d,K.classList.add("ll-cell-hover"));let c=parseInt(d.dataset.row),x=parseInt(d.dataset.layer);Kt(c,x,i.clientX,i.clientY)}),C.addEventListener("mouseleave",()=>{K&&(K.classList.remove("ll-cell-hover"),K=null),Ne()}),C.addEventListener("click",i=>{let d=i.target.closest(".ll-cell");if(d){let x=parseInt(d.dataset.row),b=parseInt(d.dataset.layer);Ne(),tn(x,b,d);return}let c=i.target.closest(".ll-row-grid");c&&ht(parseInt(c.dataset.row))}),C.addEventListener("keydown",i=>{if(i.key!=="ArrowDown"&&i.key!=="ArrowUp")return;i.preventDefault();let d=s.selectedRow??-1,c=i.key==="ArrowDown"?Math.min(M-1,d+1):Math.max(0,d-1);ht(c,void 0,!0)}),ce.addEventListener("click",i=>{let d=i.target.closest("[data-nav]");if(!d||d.hasAttribute("disabled"))return;let c=d.dataset.nav;c==="panL"?Re(-Math.max(1,Math.floor(s.viewSize/4))):c==="panR"?Re(Math.max(1,Math.floor(s.viewSize/4))):c==="zoomIn"?pt(-1):c==="zoomOut"?pt(1):c==="reset"&&qt()});function Ve(){A(),J(),re(),requestAnimationFrame(()=>{m||(A(),J(),he||Fe())})}function Ue(){Ze(D,v(),R)}function me(){L("stateChange",vt())}Ve(),We(),Ue();let Ge=0,ft=l?.clientWidth??0,gt=l?.clientHeight??0,bt=new ResizeObserver(()=>{let i=l?.clientWidth??0,d=l?.clientHeight??0;i===ft&&d===gt||(ft=i,gt=d,!Ge&&(Ge=requestAnimationFrame(()=>{Ge=0,!m&&(A(),he||Fe())})))});l&&bt.observe(l);let on=Ce(l,i=>{s.darkModeOverride===null&&(Ze(D,i,R),A(),J(),re())});function vt(){return{ramp:s.ramp,showGrid:s.showGrid,dimLowProb:s.dimLow,selectedRow:s.selectedRow,selectedLayer:s.selectedLayerIdx,viewStart:s.viewStart,viewSize:s.viewSize,colorIndex:s.colorIndex,pinnedGroups:s.pinned.map(i=>({tokens:[i.token],color:i.color})),darkMode:s.darkModeOverride}}return{widget:{getState:vt,setState:i=>{i.ramp!==void 0&&(s.ramp=i.ramp),i.showGrid!==void 0&&(s.showGrid=i.showGrid),i.dimLowProb!==void 0&&(s.dimLow=i.dimLowProb),i.selectedRow!==void 0&&(s.selectedRow=i.selectedRow),i.selectedLayer!==void 0&&(s.selectedLayerIdx=i.selectedLayer),i.viewStart!==void 0&&(s.viewStart=i.viewStart),i.viewSize!==void 0&&(s.viewSize=i.viewSize),i.colorIndex!==void 0&&(s.colorIndex=i.colorIndex),i.pinnedGroups!==void 0&&(s.pinned=f(i.pinnedGroups)),i.darkMode!==void 0&&(s.darkModeOverride=i.darkMode),z(),Ue(),Ve()},setData:i=>{u=Qe(i),a=u.normalized,h=u.v2Data,z(),s.selectedRow=null,s.selectedLayerIdx=null,Ve(),We()},setTitle:()=>{},setThemeMode:i=>{s.darkModeOverride=!!i,Ue(),A(),J(),re()},getThemeMode:()=>v(),hasEntropyData:()=>!!h&&Array.isArray(h.entropy)&&h.entropy.length>0,hasRankData:()=>{if(!h?.tracked)return!1;for(let i of h.tracked)for(let d in i){let c=i[d];if(c&&typeof c=="object"&&Array.isArray(c.rank))return!0}return!1},linkColumnsTo:()=>{},unlinkColumns:()=>{},on:(i,d)=>{(g[i]||(g[i]=[])).push(d)},off:(i,d)=>{g[i]=(g[i]||[]).filter(c=>c!==d)},destroy:()=>{m=!0,on(),bt?.disconnect(),je(),R.remove(),X&&(X.destroy(),X=null),l&&(l.innerHTML="")}},styleEl:r}}var En='<svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round"><path d="M3 12a9 9 0 1 0 9-9 9.75 9.75 0 0 0-6.74 2.74L3 8"/><path d="M3 3v5h5"/></svg>',Sn='<svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round"><path d="m15 18-6-6 6-6"/></svg>',$n='<svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round"><path d="m9 18 6-6-6-6"/></svg>',Cn='<svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round"><path d="M12 5v14M5 12h14"/></svg>',Hn='<svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round"><path d="M5 12h14"/></svg>';var _e=class{constructor(e,o,n){this.widget=null;this.styleEl=null;let l=zt(e,o,n);l&&(this.widget=l.widget,this.styleEl=l.styleEl)}getState(){return this.widget?.getState()??{}}setState(e){this.widget?.setState(e)}setData(e){this.widget?.setData(e)}setTitle(e){this.widget?.setTitle(e)}setThemeMode(e){this.widget?.setThemeMode(e)}getThemeMode(){return this.widget?.getThemeMode()??!1}hasEntropyData(){return this.widget?.hasEntropyData()??!1}hasRankData(){return this.widget?.hasRankData()??!1}linkColumnsTo(e){this.widget&&this.widget.linkColumnsTo(e)}unlinkColumns(e){this.widget&&this.widget.unlinkColumns(e)}on(e,o){this.widget?.on(e,o)}off(e,o){this.widget?.off(e,o)}destroy(){this.widget?.destroy(),this.widget=null,this.styleEl?.parentNode&&(this.styleEl.parentNode.removeChild(this.styleEl),this.styleEl=null)}};function Me(t){let e=document.createElement("div");return e.textContent=t,e.innerHTML}function Ot(t,e,o){let n=o.cellWidth??44,l=o.rowHeaderWidth??100,u=o.darkMode??!1,a="";a+="<colgroup>",a+=`<col style="width:${l}px;">`;for(let r=0;r<e.columns.length;r++)a+=`<col style="width:${n}px;">`;a+="</colgroup>";let h=o.maxRows!=null?Math.min(e.rows.length,o.maxRows):e.rows.length;for(let r=0;r<h;r++){let f=e.rows[r];a+="<tr>";let s=`width:${l}px;max-width:${l}px;`;f.pinned&&(s+=u?"background:#4a4a00;color:#fff;":"background:#fff59d;"),a+=`<td class="hm-row-header${f.pinned?" hm-pinned":""}" data-row="${r}" title="${Me(f.label)}" style="${s}">`,a+=Me(f.label),a+="</td>";for(let g=0;g<e.columns.length;g++){let L=e.getCellValue(r,g),p=`background:${L.color};color:${L.textColor};width:${n}px;max-width:${n}px;`;L.highlighted&&L.highlightColor&&(p+=`box-shadow:inset 0 0 0 2px ${L.highlightColor};`),L.bold&&(p+="font-weight:bold;"),a+=`<td class="hm-cell${L.highlighted?" hm-highlighted":""}" data-row="${r}" data-col="${g}" style="${p}">`,a+=Me(L.text),a+="</td>"}a+="</tr>"}a+="<tr>",a+=`<th class="hm-corner" style="width:${l}px;max-width:${l}px;">${Me(o.cornerLabel??"Layer")}</th>`;for(let r=0;r<e.columns.length;r++)a+=`<th class="hm-col-header" style="width:${n}px;max-width:${n}px;">${Me(e.columns[r].label)}</th>`;return a+="</tr>",a}function Pn(t){return`
        .heatmap-${t} {
